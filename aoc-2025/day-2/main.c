...
int main() {
  ...
  getline(&line, &len, fp);
  char** list = str_split(line, len, ',');
  
  long long sum = 0;

  for (int i = 0; i < count_delim(line, len, ',')+1; i++) {
    printf("--- %s ---\n", list[i]);
    int n1,n2;
...
char** str_split(char*str, int len, char delim) {
  if (len == 0) return 0; // null check

  int n_delim = count_delim(str, len, ',');
  char** out = malloc((n_delim+1) * sizeof(char*));
  int c = 0;
  int prev_idx = 0;
  out[c] = (char*)malloc((chars_till_delim(str, 0, len, delim)+1) * sizeof(char*)); // allocate first array item
  for (int i = 0; i < len; i++) {
    if (str[i] == delim) {
      out[c][i+1] = '\0';
      prev_idx = i+1;
